...
export const getUserById = (dataemail) => async dispatch => {

    dispatch({ type: 'GET_USERBYID_REQUEST' })

    try {
        const response = await axios.post('/api/getid', { dataemail })
        console.log(response);
...
export const editUser = (newuser) => async dispatch => {
    ...
    try {
        const response = await axios.post('/api/edituser', newuser)
        console.log(response);
